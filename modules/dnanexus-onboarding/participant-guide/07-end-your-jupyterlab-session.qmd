...
::: {.callout-note title="You may see a connection error"}
After ending the session, JupyterLab may display a **Server Connection Error**. This is expected: the JupyterLab server has been stopped, so the browser can no longer connect to it.

You can close the JupyterLab browser tab after the session has ended.
:::
